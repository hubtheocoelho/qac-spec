...
The commit message contains **only** the subject line and the trailer block. Do not add a body paragraph between them — `What` and `Why` are the body. Writing prose before the trailers breaks the fixed structure and pushes the trailers out of view.
...
- No body paragraph — the message is the subject line plus the trailer block, nothing between them
...
- [ ] No body paragraph between the subject and the trailers
